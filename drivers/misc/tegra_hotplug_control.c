...
static ssize_t min_time_g_cluster_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%u\n", get_min_time_g_cluster());
}

static ssize_t min_time_g_cluster_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    unsigned int new_val;
    
        sscanf(buf, "%u", &new_val);
    
    if (new_val != get_min_time_g_cluster() && new_val >= 0 && new_val <= 100)
    {
        update_min_time_g_cluster(new_val);
    }
...
static DEVICE_ATTR(min_time_g_cluster, 0777, min_time_g_cluster_show, min_time_g_cluster_store);
...
static struct attribute *mako_hotplug_control_attributes[] =
{
    	...
    	&dev_attr_min_time_g_cluster.attr,
